...
//    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        session = new SessionManager(RequestDetailActivity.this);

        font = Typeface.createFromAsset(RequestDetailActivity.this.getAssets(),"fonts/Nexa Light.otf");
        fontbold = Typeface.createFromAsset(RequestDetailActivity.this.getAssets(),"fonts/Nexa Bold.otf");
...
//        temp.put("status_name",object.getString("status_name")) ;
        ...
        isi.setText("Waktu Order : "+ data.get("order_date")+"\n\n"+
                ...
                "Catatan : "+ data.get("notes").replace("null","-")+"\n"+
...
//                    pertamaa.setVisibility(View.GONE);
               // else if (visibility == View.VISIBLE)
                 //   pertamma.setVisibility(View.GONE);
                final EditText input = new EditText(RequestDetailActivity.this);
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Toast.makeText(RequestDetailActivity.this,"Data telah disubmit",Toast.LENGTH_SHORT).show();
                finish();
            }
        });
        ...
        LinearLayout lin2 = findViewById(R.id.lin2);

        if(data.get("status_name").equals("Sending to FM/BM") && session.getRoleId().equals("14")){
    ...
    private void showList() {
        ...
        StringRequest request = new StringRequest(Request.Method.GET, ConstantUtils.URL.SERVER+"api_data/scListItemArea/"+data.get("order_sc_number"),
                            ...
                            if(session.getRoleId().equals("12") || session.getRoleId().equals("98")){
                                ...
                                listmenu2.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                    ...
                                    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                                        ...
                                        Intent in = new Intent(RequestDetailActivity.this,RequestOrderTransActivity.class);
    ...
    private void submitUpdate(String status,String reason) {
        ...
        final ProgressDialog pd = new ProgressDialog(RequestDetailActivity.this);
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if(status.equals("1") || status.equals("2")){
                                Toast.makeText(RequestDetailActivity.this,"Data berhasil di update",Toast.LENGTH_LONG).show();
                                finish();
                            }else{
                                Toast.makeText(RequestDetailActivity.this,"Data gagal di update",Toast.LENGTH_LONG).show();
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                            pd.dismiss();
                            Toast.makeText(RequestDetailActivity.this,e.toString(),Toast.LENGTH_LONG).show();
                        }
                    }
                },
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError volleyError) {
                        pd.dismiss();
                        Toast.makeText(RequestDetailActivity.this, "Periksa Koneksi Internet Anda", Toast.LENGTH_SHORT).show();
                    }
                });
        // Adding JsonObject request to request queue
        System.out.println(request);
        AppSingleton.getInstance(RequestDetailActivity.this).addToRequestQueue(request, REQUEST_TAG);
    }

    private void submitKonfirm(String status,String reason) {
        ...
        StringRequest request = new StringRequest(Request.Method.GET, ConstantUtils.URL.SERVER+"api_data/confirmReceived/"+idOrder+"/"+data.get("id_alamat")+"/"+idUser,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
    ...
    private void submitKomplain(String status,String reason) {
        ...
        // String idUser = session.getId();
        String idOrder = data.get("order_sc_number");
        String id = data.get("id_order_sc");
        String role = session.getRoleId();
        String idUser = session.getId();

        StringRequest request = new StringRequest(Request.Method.GET, ConstantUtils.URL.SERVER+"api_data/setComplaint/"+idOrder+"/"+id+"/"+reason+"/"+data.get("id_alamat")+"/"+idUser,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
